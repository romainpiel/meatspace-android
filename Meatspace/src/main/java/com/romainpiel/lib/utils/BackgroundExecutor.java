...
public class BackgroundExecutor {
	 ...
	 *             {@link #setExecutor(java.util.concurrent.Executor)} has been called with such an
	 *             executor)
    ...
    public static void execute(final Runnable runnable, final Runnable onCompleteOnUiThread) {
        directExecute(new Runnable() {
            @Override
            public void run() {
                runnable.run();
                handler.post(onCompleteOnUiThread);
            }
        }, 0);
    }
	 ...
	 * Change the executor.
	 * 
	 * Note that if the given executor is not a {@link java.util.concurrent.ScheduledExecutorService}
	 * then executing a task after a delay will not be supported anymore. If it
	 * is not even a {@link java.util.concurrent.ExecutorService} then tasks will not be cancellable
	 * anymore.
	 * 
	 * @param executor
	 *            the new executor
